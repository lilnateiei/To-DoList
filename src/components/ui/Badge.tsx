...
const CATEGORY_CONFIG: Record<Category, { label: string; className: string }> = {
  ...
  development: { label: "💻 Dev",         className: "bg-surface-1 text-ink border border-hairline"   },
...
const PRIORITY_CONFIG: Record<Priority, { label: string; className: string }> = {
  low:    { label: "ต่ำ",   className: "bg-surface-1 text-success border border-hairline" },
  medium: { label: "กลาง", className: "bg-surface-1 text-warning border border-hairline"   },
  high:   { label: "สูง",   className: "bg-surface-1 text-error border border-hairline"     },
};
